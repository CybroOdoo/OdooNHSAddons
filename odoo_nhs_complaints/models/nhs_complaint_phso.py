...
class NhsComplaintPhso(models.Model):
    ...
    action_ids = fields.One2many('nhs.action', 'phso_id',
                                 string='Actions from Recommendations',
        ...
        for rec in self:
            if rec.state in ('referred', 'under_review'):
                if rec.outcome or rec.compensation_recommended or rec.recommendations:
                    raise ValidationError('Outcome, compensation, and recommendations'
                                          ' can only be set once a decision is made.')
