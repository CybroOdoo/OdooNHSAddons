...
class NhsDsptAssertion(models.Model):
    ...
    name = fields.Char(
        ...
        store=True,
        help="Name of the assertion derived from its definition."
    )
    reference = fields.Char(
        string='Reference',
        related='assertion_def_id.reference',
        store=True,
        help="Reference code of the assertion."
    )
    ...
    standard_id = fields.Many2one(
        ...
        store=True,
        help="Standard under which this assertion falls."
    )
    ...
    evidence_count = fields.Integer(
        ...
        help="Total number of evidence items linked to this assertion."
    )
    mandatory_met_count = fields.Integer(
        string='Mandatory Met',
        compute='_compute_evidence_count',
        help="Number of mandatory evidence items marked as Met."
    )
    mandatory_total_count = fields.Integer(
        string='Mandatory Total',
        compute='_compute_evidence_count',
        help="Total number of mandatory evidence items applicable to this assertion."
    )
    note = fields.Text(
        string='Notes',
