...
class NhsDsptAssessment(models.Model):
    ...
    year = fields.Char(
        ...
        store=True,
        help="Toolkit edition year string (e.g. 2025/26)."
    )
    ...
    state = fields.Selection([
        ...
        ('published', 'Published'),
        ('submitted', 'Submitted'),
    ], string='Status', required=True, default='draft', tracking=True, help="Current lifecycle state of the assessment.")
    ...
    action_ids = fields.One2many(
        ...
        help="Improvement actions raised to resolve compliance gaps."
    )
    action_count = fields.Integer(
        string='Action Count',
        compute='_compute_action_count',
        help="Total number of improvement actions associated with this assessment."
    )
    evidence_count = fields.Integer(
        string='Evidence Count',
        compute='_compute_action_count',
        help="Total number of evidence items in this assessment."
    )
    ...
    stale_evidence_count = fields.Integer(
        ...
        compute='_compute_readiness',
        store=True,
        help="Number of evidence items marked as stale (evidence review date expired)."
    )
    deadline = fields.Date(
        string='Deadline',
        related='edition_id.deadline',
        store=True,
        help="Submission deadline date for the assessment edition."
    )
    published_by_id = fields.Many2one(
        'res.users',
        string='Published By',
        readonly=True,
        help="User who marked this assessment as published."
    )
    ...
    published_at = fields.Datetime(
        string='Published At',
        readonly=True,
        help="Date and time when the assessment was published."
    )
    ...
    submission_date = fields.Date(
        ...
        help="Date on which the assessment was submitted to the NHS portal."
    )
    ...
    active = fields.Boolean(
        string='Active',
        default=True,
        help="Indicates whether this assessment is active or archived."
    )
