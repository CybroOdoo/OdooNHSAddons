...
class NhsDsptBulkOwnerWizard(models.TransientModel):
    ...
    active_model = fields.Char(
        ...
        help="Model type of the selected records."
    )
    record_count = fields.Integer(
        string='Records Selected',
        default=lambda self: len(self.env.context.get('active_ids') or []),
        help="Number of records selected for bulk owner reassignment."
    )
    ...
    def _default_active_model(self):
        """Determines the active model from context and ensures it is supported for bulk owner assignment."""
    ...
    def action_confirm(self):
        """Reassigns the selected user owner across all selected records."""
        self.ensure_one()
