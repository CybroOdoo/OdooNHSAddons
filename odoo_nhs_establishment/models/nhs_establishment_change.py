...
class NhsEstablishmentChange(models.Model):
    ...
    def _compute_cost_impact(self):
        """Estimate the annual pay-cost impact of the proposed change."""
        for change in self:
    ...
    @api.onchange('proposed_contracted_hours', 'proposed_headcount')
    def _onchange_proposed_fte_basis(self):
        """Recompute proposed FTE from proposed hours and headcount."""
        basis = self.company_id.nhs_full_time_hours_basis or 37.5
        if self.proposed_contracted_hours:
            self.proposed_fte = self.env['nhs.establishment.post']._compute_fte_value(
                self.proposed_contracted_hours, self.proposed_headcount, basis)

    @api.onchange('post_id')
    def _onchange_post_id(self):
        """Prefill proposed values from the selected post."""
        for change in self:
            if change.post_id and change.change_type in ('increase_fte', 'decrease_fte', 'reband'):
                change.proposed_fte = change.post_id.funded_fte
                change.proposed_headcount = change.post_id.funded_headcount
                change.proposed_contracted_hours = change.post_id.contracted_hours
                change.proposed_band_id = change.post_id.band_id
                change.proposed_is_medical = change.post_id.is_medical
                change.proposed_manual_indicative_salary = change.post_id.manual_indicative_salary

    @api.constrains('change_type', 'post_id', 'org_unit_id')
    def _check_required_refs(self):
        """Require the post/target unit relevant to the change type."""
        for change in self:
    ...
    def create(self, vals_list):
        """Sequence the reference number when not provided."""
    ...
    def action_submit(self):
        """Move draft change requests to Submitted."""
        for change in self:
    ...
    def _check_can_approve(self):
        """Require the Workforce Manager group to approve or apply changes."""
    ...
    def action_workforce_approve(self):
        """Workforce-approve the change, skipping to Finance Approved for single-stage companies."""
    ...
    def action_finance_approve(self):
        """Finance-approve a workforce-approved change."""
        self._check_can_approve()
        for change in self:
    ...
    def action_apply(self):
        """Apply a finance-approved change to the establishment."""
        self._check_can_approve()
        for change in self:
    ...
    def action_reject(self):
        """Reject a pending change request, requiring a rejection reason."""
        for change in self:
    ...
    def _apply_change(self):
        """Carry out the approved change on the affected post."""
        self.ensure_one()
        ...
        if self.change_type == 'create_post':
            ...
            before_fte = post.funded_fte
            before_headcount = post.funded_headcount
            post.write({
                'funded_fte': self.proposed_fte,
                'funded_headcount': self.proposed_headcount,
            })
            post.message_post(
                body=f"Funded FTE changed by Change {self.name}: "
                     f"{before_fte:.2f} -> {self.proposed_fte:.2f} "
                     f"(headcount {before_headcount} -> {self.proposed_headcount}).")
