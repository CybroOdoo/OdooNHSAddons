...
class NhsEstablishmentPost(models.Model):
    ...
    def _compute_name(self):
        """Build the display name from job title and org unit."""
        for post in self:
    ...
    def _compute_vacant(self):
        """Work out the vacant FTE and vacancy status for each post."""
        for post in self:
    ...
    def _compute_days_vacant(self):
        """Work out how many days the current vacancy has lasted."""
    ...
    def _compute_indicative_pay(self):
        """Estimate annual pay cost from band/manual salary, FTE and on-costs."""
        for post in self:
    ...
    def _compute_change_count(self):
        """Count change requests raised against each post."""
    ...
    def _onchange_fte_basis(self):
        """Recompute funded and in-post FTE when hours or headcount change."""
    ...
    def _onchange_org_unit_id(self):
        """Default the cost centre from the selected org unit."""
    ...
    def _onchange_is_medical(self):
        """Clear the AfC band when the post is marked Medical / Non-AfC."""
        if self.is_medical:
            self.band_id = False

    @api.constrains('funded_fte', 'in_post_fte')
    def _check_fte_non_negative(self):
        """Block negative funded or in-post FTE values."""
        for post in self:
    ...
    def _check_band_required(self):
        """Require an AfC band unless the post is Medical / Non-AfC or deleted."""
        for post in self:
    ...
    def create(self, vals_list):
        """Sequence the reference and sync active/frozen flags with status."""
    ...
    def write(self, vals):
        """Enforce change control on FTE/band/team edits and keep status flags in sync."""
    ...
    def _update_vacancy_start_date(self):
        """Stamp or clear the vacancy start date as posts become vacant or filled."""
        today = fields.Date.context_today(self)
        for post in self:
    ...
    def action_raise_change_request(self):
        """Open the wizard to raise a change request for this post."""
        self.ensure_one()
        return {
    ...
    def action_view_changes(self):
        """Open the change-request history for this post."""
        self.ensure_one()
        return {
            'name': 'Change History',
            'type': 'ir.actions.act_window',
