...
class NhsOrgUnit(models.Model):
    ...
    def _compute_complete_name(self):
        """Build the breadcrumb name from the parent chain."""
        for unit in self:
            if unit.parent_id:
    ...
    def _compute_establishment_totals(self):
        """Roll up FTE and vacancy figures for this unit and its descendants."""
        for unit in self:
    ...
    def _compute_post_count(self):
        """Count posts under this unit, including sub-units."""
        for unit in self:
    ...
    def _check_parent_recursion(self):
        """Block a unit hierarchy that loops back on itself."""
    ...
    def _check_archive_with_live_posts(self):
        """Block archiving a unit that still has active/frozen posts."""
        for unit in self:
            if unit.active:
                continue
    ...
    def unlink(self):
        """Block deleting a unit that still has posts or sub-units attached."""
        for unit in self:
    ...
    def create(self, vals_list):
        """Auto-assign a unit code from the sequence when not provided."""
    ...
    def action_view_posts(self):
        """Open the posts under this unit, including sub-units."""
        self.ensure_one()
        return {
            'name': 'Posts',
