...
class NhsEstablishmentPost(models.Model):
    """Adds training requirement profile and member tracking to posts."""
    ...
    member_count = fields.Integer(
        ...
        help="Number of workforce members currently holding this post."
    )

    def _compute_member_count(self):
        """Count the workforce members currently holding this post."""
        for post in self:
            post.member_count = len(post.member_ids)

    def action_view_workforce_members(self):
        """Open the workforce members currently holding this post."""
        self.ensure_one()
        return {
