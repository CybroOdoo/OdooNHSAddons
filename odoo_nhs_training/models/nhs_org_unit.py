...
class NhsOrgUnit(models.Model):
    """Adds workforce member counts and training compliance roll-ups to org units."""
    ...
    team_compliant_count = fields.Integer(
        ...
        store=True,
        recursive=True,
        help="Sum of compliant subjects across members of this unit and its descendants."
    )
    ...
    def _compute_member_count(self):
        """Count live (non-leaver) members in the unit and its descendants.
        sudo() deliberately - this is just a count, not exposing individual
        member records, and must not raise an access error for viewers
        (e.g. job-planning's Clinical Manager) who can see the org unit but
        have no training-module access at all; the "Training Members"
        button itself stays groups()-gated so those users still can't drill
        into the actual member records."""
        for unit in self:
            unit.member_count = self.env['nhs.workforce.member'].sudo().search_count([
    ...
    def _compute_team_compliance(self):
        """Roll up required/compliant subject counts and compliance % from members and child units."""
        for unit in self:
    ...
    def action_view_workforce_members(self):
        """Open the workforce members belonging to this unit and its descendants."""
        self.ensure_one()
        return {
    ...
    def _cron_send_weekly_digest(self):
        ...
        for unit in units:
            ...
            self.env['mail.mail'].sudo().create({
                ...
                'subject': ('Weekly Training Compliance Digest — %s') % unit.complete_name,
                'body_html': (
        ...
        if fallback:
            ...
            self.env['mail.mail'].sudo().create({
                'email_to': fallback,
                'subject': ('Weekly Training Compliance Digest — Organisation'),
