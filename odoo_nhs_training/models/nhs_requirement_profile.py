...
class NhsRequirementProfile(models.Model):
    """A bundle of training requirements assignable to posts and members."""
    ...
    requirement_count = fields.Integer(
        ...
        help="Number of training requirements defined on this profile."
    )
    ...
    def _compute_requirement_count(self):
        """Count the training requirements defined on this profile."""
        for profile in self:
            profile.requirement_count = len(profile.requirement_ids)

    def _compute_post_count(self):
        """Count the posts currently assigned this profile."""
        for profile in self:
            profile.post_count = len(profile.post_ids)

    def _compute_member_count(self):
        """Count the workforce members currently on this profile."""
    ...
    def action_view_posts(self):
        """Open the posts assigned this profile."""
        self.ensure_one()
        return {
            'name': 'Posts',
    ...
    def action_view_members(self):
        """Open the workforce members currently on this profile."""
        self.ensure_one()
        return {
            'name': 'Members',
            'type': 'ir.actions.act_window',
    ...
    def action_assign_posts(self):
        """Open the wizard to bulk-assign this profile to posts."""
        self.ensure_one()
        return {
            'name': 'Assign Posts',
            'type': 'ir.actions.act_window',
