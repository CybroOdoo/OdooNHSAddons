...
class NhsWorkforceMemberComplianceLine(models.Model):
    """Per-subject compliance status line for a workforce member."""
    ...
    member_id = fields.Many2one(
        ...
        ondelete='cascade',
        index=True,
        help="Workforce member this compliance line belongs to."
    )
    subject_id = fields.Many2one(
        'nhs.training.subject',
        string='Subject',
        help="Training subject this line reports the status of."
    )
    subject_name = fields.Char(
        related='subject_id.name',
        string='Subject Name',
        store=True,
        help="Stored copy of the subject's name, for list/search display."
    )
    status = fields.Selection([
        ...
        help="Member's current status against this subject, as last recomputed"
             " by the member's compliance calculation.")
    expiry_date = fields.Date(
        string='Expiry',
        help="Expiry date of the member's latest completion for this subject,"
             " if any."
    )


class NhsWorkforceMember(models.Model):
    """Light staff record for training-compliance tracking."""
    ...
    def _onchange_user_id_email(self):
        """Default the email from the linked user, if one is set."""
    ...
    record_count = fields.Integer(
        ...
        help="Number of training records logged for this member."
    )
    ...
    registration_count = fields.Integer(
        ...
        help="Number of professional registrations logged for this member."
    )
    ...
    compliant_subject_count = fields.Integer(
        ...
        compute='_compute_compliance',
        store=True,
        help="Required subjects (excluding exempt) currently in date, i.e."
             " status compliant or due_soon."
    )
    expired_subject_count = fields.Integer(
        string='Expired Subjects',
        compute='_compute_compliance',
        store=True,
        help="Required subjects whose latest attempt has passed its expiry"
    ...
    def _onchange_post_id(self):
        """Default team, staff group and requirement profile from the post."""
        for member in self:
            if member.post_id:
    ...
    def create(self, vals_list):
        """Assign a sequence reference, default team/staff group/requirement
        profile from the chosen post when not explicitly set, then prompt
        induction training for the newly-created members.
        """
    ...
    def write(self, vals):
        """When the post changes, cascade its team, staff group and
        requirement profile onto the member unless those fields are also
        being set explicitly in the same call; clear them when the post is
        removed.
        """
        if 'post_id' in vals:
            if vals.get('post_id'):
    ...
    def _prompt_induction_training(self):
        """Schedule a to-do activity on each member of self to arrange
        induction/mandatory (one-off) training, if any such subjects are
        configured and the activity type is available.
        """
        ...
        for member in self:
            member.activity_schedule(
                ...
                summary=('New starter — arrange induction training'),
                note=('Arrange induction/mandatory training for %s (%s).') % (
    ...
    def _compute_compliance(self):
        """Compute each member's required/compliant/expired/failed subject
        counts, compliance percentage and overall compliance status, then
        sync `compliance_line_ids` (in memory for new/unsaved records, or via
        create/write/unlink against the database for saved ones) so it
        mirrors the current per-subject statuses and expiry dates.
        """
    ...
    def _compute_required_subject_ids(self):
        """Resolve the set of subjects currently required of each member,
        from their profile/staff-group requirements and individual overrides.
        """
        lines_by_member = self.get_requirement_lines()
        for member in self:
    ...
    def action_view_registrations(self):
        ...
        self.ensure_one()
        return {
            'name': ('Professional Registrations'),
            'type': 'ir.actions.act_window',
    ...
    def _cron_recompute_compliance(self):
        """Scheduled action: refresh record and registration statuses, then
        recompute member and org-unit compliance so due/expired items are
        picked up without waiting for an on-demand write.
        """
    ...
    def get_import_templates(self):
        """Import templates offered on the workforce member import wizard."""
        return [{
