...
class NhsProfileAssignPostsWizard(models.TransientModel):
    """Wizard for linking active posts to a requirement profile."""
    ...
    profile_id = fields.Many2one(
        ...
        required=True,
        help="The requirement profile that the selected posts will be assigned to."
    )
    ...
    def _onchange_profile_id(self):
        """Pre-select the posts already linked to the chosen profile."""
        for wizard in self:
            if wizard.profile_id:
                wizard.post_ids = wizard.profile_id.post_ids

    def action_confirm(self):
        """Assign the selected posts to the profile and unlink any deselected ones.
        """
        self.ensure_one()
